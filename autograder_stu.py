...
def check_cage(csp, grid, var):
    ...
    for cage in to_be_checked:
        ...
        if cage.assignedValue == '+':
            ...
            if not mod_valid(cage_dict[cage.name][0], assigned_values):
                print('fail on mod ' + cage.name)
                return False
        else:
            if len(cage_dict[cage.name][1]) == 1 and cage_dict[cage.name][1][0].assignedValue != cage_dict[cage.name][0]:
                ...
                and not mul_valid(cage_dict[cage.name][0], assigned_values) and not div_valid(cage_dict[cage.name][0], assigned_values) and not mod_valid(cage_dict[cage.name][0], assigned_values):
...
class TestCageConstraints(unittest.TestCase):
    ...
    def test_cages_7(self):
        ...
        board = (2, [(1, [(1, 1), (1, 2), (2, 1), (2, 2)], '%')])
        correct = test_cages_7_correct()
        ...
        guess = [c for c in csp.get_all_cons() if len(c.scope) == 5]
        if not guess:
            self.fail("No cage constraints found with scope length 5 (4 cells + operator).")
        answered = []
        used_guess = []
...
class TestPropGAC(unittest.TestCase):
    ...
    def test_prop_GAC_10(self):
        self.prop_GAC_helper(10)
...
def main(verbosity=2):
    ...
    with open(file_name, "a") as out:
        ...
        cage_score = (CAGE_POINTS/13.5)*CAGE_FULL
        fc_score = (FC_POINTS/10.0)*FC_FULL
        gac_score = (GAC_POINTS/11.0)*GAC_FULL
